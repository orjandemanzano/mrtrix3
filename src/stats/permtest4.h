...
/////////////////	      
	    Eigen::MatrixXf fillPermsMatrix(){
		Eigen::MatrixXf perms;
		int ndim = 8;
		int nperms = pow(2,ndim);
		perms.resize(nperms,1);
		for (int i = 0; i < nperms/2; i++){
		    perms(i,0) = 1;
		}
		for (int i = nperms/2; i < nperms; i++){

		    perms(i,0) = -1;
		}
		Eigen::MatrixXf tmpcol;
		tmpcol.resize(nperms,1);
		for (int k = 0; k < ndim-1; k++) {
		    int j = 0;
		    for (int i = 0; i < nperms; i+=2){
		    tmpcol(j,0) = perms(i,k);
		    j++;
		    }
		tmpcol.block(nperms/2,0,nperms/2,1) = tmpcol.block(0,0,nperms/2,1);
		perms.conservativeResize(nperms,perms.cols()+1);
		perms.block(0,k+1,nperms,1) = tmpcol;
		}
		return perms;
	    }
 ...
 /////////////////	    
            ...
            {
              size_t index;
	      Eigen::MatrixXf permsMatrix = fillPermsMatrix();
		...
		for (int i = 0; i < 8; ++i) {
		      perm.push_back(permsMatrix(index,i));
		}
		process_permutation (index, perm);
	      }
            }

          protected:

            void process_permutation (size_t index, std::vector<size_t> p)
            {
              value_type max_stat = 0.0, min_stat = 0.0;
              stats_calculator (p, stats, max_stat, min_stat); //////////////////
...
/////////////////	      
 ...
 /////////////////	      
              ...
              {
                size_t index;
		Eigen::MatrixXf pMatrix = fillPMatrix();
		std::ofstream outputFile;
	    	outputFile.open("C:/msys64/home/Orjan/mrtrix3/lib/math/stats/DataOutProcessor.txt", std::fstream::out | std::fstream::trunc);
            	if (outputFile.is_open()) {
                	outputFile << pMatrix << "\n" << std::flush;
            	}
            	outputFile.close();
		...
		std::ofstream outputFile1;
	    	outputFile1.open("C:/msys64/home/Orjan/mrtrix3/lib/math/stats/DataOutProcessorHello.txt", std::fstream::out | std::fstream::app);
		...
		while (( index = perm_stack.next() ) < perm_stack.num_permutations) {
			std::ofstream outputFile2;
	    		outputFile2.open("C:/msys64/home/Orjan/mrtrix3/lib/math/stats/DataOutIndex.txt", std::fstream::out | std::fstream::app);
		    ...
		    std::vector<size_t> perm;
		    for (int i = 0; i < 8; ++i) {
		      perm.push_back(pMatrix(index,i));
		    }
		    process_permutation (index, perm);
		}
              }
            ...
            protected:
              ...
              {
                value_type max_stat = 0.0, min_stat = 0.0;
                stats_calculator (p, statistics, max_stat, min_stat);
        ...
        template <class StatsType, class EnhancementType>
          ...
          template <class StatsType, class EnhancementType>
                                                        ...
                                                        std::vector<value_type>& default_statistics)
            {
              ...
              value_type max_stat = 0.0, min_stat = 0.0;
////////////
	      std::vector<size_t> permFirstRow;
	      permFirstRow = {1,1,1,1,1,1,1,1};
////////////
              stats_calculator (permFirstRow, default_statistics, max_stat, min_stat);
