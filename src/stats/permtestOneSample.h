...
class PermsMatrix {
	public:
	    static Eigen::MatrixXf fillPermsMatrix(size_t ndim){
		Eigen::MatrixXf perms;
		size_t nperms = pow(2,ndim);
		perms.resize(nperms,1);
		for (size_t i = 0; i < nperms/2; i++){
		    perms(i,0) = 1;
		}
		for (size_t i = nperms/2; i < nperms; i++){
      ...
      template <class StatsType, class EnchancementType>
		...
		for (int i = 0; i < permsMatrix.cols(); ++i) {
        ...
        template <class StatsType, class EnhancementType>
          ...
          template <class StatsType, class EnhancementType>
                                                        ...
                                                        std::vector<value_type>& default_statistics)
            {
              ...
              value_type max_stat = 0.0, min_stat = 0.0;
////////////
	      std::vector<size_t> permFirstRow (stats_calculator.num_subjects());
	      std::fill (permFirstRow.begin(),permFirstRow.end(),1);
////////////
