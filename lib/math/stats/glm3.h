    ...
    namespace Stats
    {
      ...
      class OneSampleGLMTTest
      {
        public:
          ...
          * @param contrast a matrix containing the contrast of interest.
          */
          OneSampleGLMTTest (const Eigen::MatrixXf& measurements,
                    const Eigen::MatrixXf& design,
                    const Eigen::MatrixXf& contrast) :
            y (measurements),
            X (design),
          ...
          * @param min_stat the minimum t-statistic
          */
          void operator() (const std::vector<size_t>& perm_vec, std::vector<float>& stats,
                           float& max_stat, float& min_stat) const
          {
            stats.resize (y.rows(), 0.0);
            Eigen::MatrixXf tvalues, betas, residuals, SX, pinvSX;
            ...
            for (ssize_t i = 0; i < X.rows(); ++i) {
              SX.row(i) = X.row (i);
              pinvSX.col(i) = pinvX.col(i);
            }
            pinvSX.transposeInPlace();
            SX.transposeInPlace();

	    Eigen::MatrixXf permRow(1,8);
	    for (int i = 0; i < 8; ++i){
		    if (perm_vec[i] != 1)
		    	permRow(0,i) = -1;
		    else
	    ...
	    outputFile1.open("C:/msys64/home/Orjan/mrtrix3/lib/math/stats/DataOutPermVec.txt", std::fstream::out | std::fstream::app);
            if (outputFile1.is_open()) {
		    for (int i = 0; i < 8; ++i) {
                	outputFile1 << perm_vec[i] << " " << std::flush;
		    }
		    outputFile1 << "\n" << std::flush;
            }
            outputFile1.close();
	    ...
	    outputFile2.open("C:/msys64/home/Orjan/mrtrix3/lib/math/stats/DataOutPermRow.txt", std::fstream::out | std::fstream::app);
	      ...
	      for (ssize_t j = 0; j < tmp.rows(); ++j) { 
		     Eigen::MatrixXf tmpData = tmp.row(j);
		     tmp.row(j) = tmpData.cwiseProduct(permRow);
	      }
